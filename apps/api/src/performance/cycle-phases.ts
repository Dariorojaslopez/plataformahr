...
export type CyclePhaseSource = {
  ...
  startDate: Date | string;
  endDate: Date | string;
  evaluationStartDate?: Date | string | null;
  evaluationEndDate?: Date | string | null;
  goalDefinitionStartDate?: Date | string | null;
  goalDefinitionEndDate?: Date | string | null;
  managerEvaluationStartDate?: Date | string | null;
  managerEvaluationEndDate?: Date | string | null;
  calibrationStartDate?: Date | string | null;
  calibrationEndDate?: Date | string | null;
  closingStartDate?: Date | string | null;
  closingEndDate?: Date | string | null;
...
export function buildCyclePhases(
  ...
  return raw.map((phase, index) => {
    if (phase.id === currentId) {
      return { ...phase, visibility: 'current' as const };
    }
    if (currentIndex >= 0) {
      const visibility: PhaseVisibility =
        index < currentIndex ? 'past' : 'future';
      return { ...phase, visibility };
    }
    const visibility: PhaseVisibility =
      phase.endDate < today ? 'past' : 'future';
    return { ...phase, visibility };
  });
}
