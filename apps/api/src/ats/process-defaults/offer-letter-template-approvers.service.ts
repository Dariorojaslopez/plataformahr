...
@Injectable()
export class OfferLetterTemplateApproversService {
  constructor(
  ...
  async get(companyId: string) {
    const steps = await this.prisma.offerLetterTemplateApprover.findMany({
      where: { companyId },
  ...
  async update(tenant: TenantContext, dto: ReplacePositionOccupantStepsDto) {
    const normalized: Prisma.OfferLetterTemplateApproverCreateManyInput[] = [];
    ...
    const steps = await this.prisma.$transaction(async (tx) => {
      await tx.offerLetterTemplateApprover.deleteMany({
        where: { companyId: tenant.companyId },
      });
      if (normalized.length > 0) {
        await tx.offerLetterTemplateApprover.createMany({ data: normalized });
      }
      return tx.offerLetterTemplateApprover.findMany({
        where: { companyId: tenant.companyId },
        orderBy: { sequence: 'asc' },
        include: APPROVER_INCLUDE,
      });
    ...
    await this.audit.create({
      action: ATS_AUDIT.OFFER_LETTER_TEMPLATE_APPROVERS_UPDATED,
      entity: 'OfferLetterTemplateApprover',
