...
export function OfferLetterTemplateApproversPageClient() {
  const companyId = useCompanyId();
  const listQuery = useQuery({
    queryKey: atsKeys.offerLetterTemplateApprovers(companyId),
    queryFn: () => atsApi.getOfferLetterTemplateApprovers(),
  });
...
function OfferLetterApproversForm({
  companyId,
  initialRows,
  positions,
}: {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      return atsApi.updateOfferLetterTemplateApprovers({
    ...
    onSuccess: async () => {
      ...
      setFormError(null);
      notifySuccess("Aprobadores de carta de oferta guardados");
    },
    onError: (error) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold">Aprobadores de carta oferta</h2>
        <p className="text-sm text-muted-foreground">
          Define quién aprueba la carta de oferta. Si el cargo tiene más de un
          ocupante, elige el nombre.
        </p>
      </div>
      <CargoOccupantListEditor
        ...
        emptyHint="Agrega los aprobadores de la carta de oferta."
      />
      <Button
        type="button"
